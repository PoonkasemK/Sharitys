...
const ViewUrgent = () => {
  ...
  const getProjects = async () => {
    try {
      const response = await fetch(
        process.env.REACT_APP_SERVER_URL + "/db/seturgentpj"
      );
  ...
  const seturgentpj = async (id) => {
    try {
      await fetch(process.env.REACT_APP_SERVER_URL + `/db/seturgentpj/${id}`, {
        method: "PUT",
      });
      alert("ตั้งเป็นโครงการเร่งด่วนเรียบร้อย");
  ...
  if (sessionStorage.getItem("role") !== "admin") {
    return (
      <Fragment>
        <NavBarChoose />
        <h1 className="text-center mt-5">ตั้งเป็นโครงการเร่งด่วน</h1>
        <br />
        <br />
        <h2 className="Omise__title">คุณไม่สามารถตั้งโครงการเร่งด่วนได้</h2>
        <Footer />
      </Fragment>
    );
  }
                ...
                <Row>
                  <Col md="auto">
                    <button
                      ...
                      onClick={() => seturgentpj(project.pjid)}
                    >
                      ตั้งเป็นโครงการเร่งด่วน
                    </button>
                  </Col>
                </Row>
              </Card.Body>
