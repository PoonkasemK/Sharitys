...
const ListProjects = () => {
  ...
  const getProjects = async () => {
    try {
      const response = await fetch(
        process.env.REACT_APP_SERVER_URL + "/db/showallexppj"
      );
  ...
  const terminateProject = async (id) => {
    try {
      await fetch(process.env.REACT_APP_SERVER_URL + `/db/terminateallpj`, {
        method: "POST",
      });
      alert("ยุติโครงการที่ถึงกำหนดทั้งหมดแล้ว");
      setProjects(projects.filter((project) => project.pjid !== id));
      window.location.href = "/TerminateExpProject";
    } catch (err) {
      console.error(err.message);
    }
  };
  ...
  if (sessionStorage.getItem("role") !== "admin") {
    return (
      <Fragment>
        <NavBarChoose />
        <h1 className="text-center mt-5">โครงการที่สิ้นสุดภายในวันนี้</h1>
        <br />
        <br />
